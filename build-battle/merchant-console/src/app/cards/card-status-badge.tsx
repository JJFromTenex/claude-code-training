...
/** Three states, kept separate from the payments StatusBadge so neither union grows for the other. */
export function CardStatusBadge({ status }: { status: CardStatus }) {
  return (
    <Badge variant={VARIANTS[status]} className="gap-1.5">
      <span className={cx("size-1.5 shrink-0 rounded-full", DOTS[status])} aria-hidden="true" />
      {LABELS[status]}
    </Badge>
  )
}
